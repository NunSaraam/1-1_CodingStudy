...
public class Expedition : MonoBehaviour
{
    ...
    void UpdataExpeditionInfo()                             //탐방 정보를 표시하는 함수
    {
    ...
    public void StartExpedition(int memberIndex)
    {
        ...
        //성공률 계산 (ExpeditionSO의 기본 성공률 + 맴버 보너스)
        int memberBouns = 0;
        ...
        if (success)
        {
            ...
            gameManager.food += currentExpedition.successFoodReward;
            ...
            resultText.text = $"{member.memberName} {currentExpedition.expeditionName} 성공 ! (성공률 : {finalSuccessRate}%)\n" +
                $"음식 + {currentExpedition.successFoodReward}, 연료 + {currentExpedition.successFuelReward}, " + $"의약품 + {currentExpedition.successMedicineReward}";
