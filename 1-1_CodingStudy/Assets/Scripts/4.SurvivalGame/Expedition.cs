...
public class Expedition : MonoBehaviour
{
    ...
    public Dropdown equipmentDropdown;                      //드롬바운 UI
    ...
    private void Start()
    {
        ...
        //내구도 배열 초기화
        InitializeEquipmentDurability();

        //드롭다운 설정 추가
        SetupEquipmentDropdown();
        equipmentDropdown.onValueChanged.AddListener(OnEquipmentChanged);           //드롭다운 선택이 변경될 때 함수를 호출한다.
    }
    ...
    void UpdateExpeditionInfo()                             //탐방 정보를 표시하는 함수
    {
        if (currentExpedition != null)
        {
            ...
            //부러진 장비는 보너스 없음
            int equipBouse = (selectedEquipmentIndex > 0 && equipmentDurability[selectedEquipmentIndex] <= 0) ? 0 : selectedEquip.successBonus;
            ...
            expeditionInfoText.text = $"탐방 : {currentExpedition.expeditionName}\n" +
                                        ...
                                        $"장비 보너스 : +{equipBouse}%{durabilityInfo}\n" +
    ...
    public void StartExpedition(int memberIndex)
    {
        ...
        int equipBonus = equipmentBroken ? 0 : selectedEquip.successBonus;
        int rewardBonus = equipmentBroken ? 0 : selectedEquip.rewardBonus;

        //성공률 계산 (ExpeditionSO의 기본 성공률 + 장비 보너스)
        int finalSuccessRate = currentExpedition.baseSuccessRate + equipBonus;
        ...
        if (success)
        {
            ...
            gameManager.food += currentExpedition.successFoodReward + rewardBonus;
            ...
            resultText.text = $"{member.memberName} {currentExpedition.expeditionName} 성공 ! (성공률 : {finalSuccessRate}%)\n" +
                $"음식 + {currentExpedition.successFoodReward + rewardBonus}, 연료 + {currentExpedition.successFuelReward + rewardBonus}, " + $"의약품 + {currentExpedition.successMedicineReward + rewardBonus}";
    ...
    void InitializeEquipmentDurability()                                    //장비 내구도 셋팅하는 함수
    {
    ...
    void SetupEquipmentDropdown()
    {
        ...
        equipmentDropdown.value = 0;                    //첫 번재 장비(맨손) 선택
        equipmentDropdown.RefreshShownValue();          //데이터 변경이 있을 경우 보여지는 값을 함수를 통해 리셋한다
    }
}
